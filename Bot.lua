...
Callback = function()
    ...
    print("[DEBUG] Raw /check response: " .. res)   -- Ye console mein dekhna zaroori hai
    ...
    if not decodeSuccess then
        Rayfield:Notify({ Title = "Error", Content = "Invalid server response", Duration = 6 })
        return
    end

    print("[DEBUG] Verified status received: " .. tostring(data.verified))
    ...
    local verified = tostring(data.verified or "pending")
